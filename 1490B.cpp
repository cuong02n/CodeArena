#include <bits/stdc++.h>
using namespace std;
int t=0;//test case

int decrease1(int a){
	if(a==0){
		return 2;
	}
	return a-1;
}
int increase1(int a){
	if(a==2){
		return 0;
	}
	return a+1;
}

int calculate(int c[],int n){
	int count=0;
	int _min=INT_MAX;
	int flag=-1;
	for(int i=0;i<3;i++){
		if(_min>c[i]){
			_min=c[i];
			flag =i;
		}
	}
	count+=abs(c[flag]-n/3);
	c[decrease1(flag)]-=count;
	c[flag]+=count;
	if(c[increase1(flag)]==n/3&&c[decrease1(flag)]==n/3){
		return count;
	}else{
		if(c[increase1(flag)]>c[decrease1(flag)]){
			count+=abs(c[increase1(flag)]-n/3);
		}else{
			count+=2*abs(c[increase1(flag)]-n/3);
		}
	}
	return count;
}
int main(){
	ios_base::sync_with_stdio(false);
	cin.tie(0);

	cin>>t;
	while(t--){
		int n;
		cin>>n;
		int c[3]={0,0,0};
		for(int i=0;i<n;i++){
			int tmp;
			cin>>tmp;
			tmp=tmp%3;
			c[tmp]++;
		}
		cout<<calculate(c,n)<<endl;
	}
	
	return 0;
}















/*

	 7MMM.     ,MMF       db       7MN.    7MF  7MMF    7MMF
	  MMMb    dPMM       ;MM:       MMN.    M    MM      MM
	  M YM   ,M MM      ,V^MM.      M YMb   M    MM      MM
	  M  Mb  M' MM     ,M  `MM      M  `MN. M    MMmmmmmmMM
	  M  YM.P'  MM     AbmmmqMA     M   `MM.M    MM      MM
	  M  `YM'   MM    A'     VML    M     YMM    MM      MM
	.JML. `'  .JMML .AMA.   .AMMA. JML.    YM  .JMML.  .JMML.

    .g8"""bgd  7MMF     7MF   .g8""8q.   7MN.   `7MF   .g8"""bgd
 .dP'     `M    MM       M  .dP'    `YM.  MMN.    M  .dP'     `M
 dM'       `    MM       M  dM'      `MM  M YMb   M  dM'       `
 MM             MM       M  MM        MM  M  `MN. M  MM
 MM.            MM       M  MM.      ,MP  M   `MM.M  MM.    `7MMF'
 `Mb.     ,'    YM.     ,M  `Mb.    ,dP'  M     YMM  `Mb.     MM
  `"bmmmd'      `bmmmmd"'    `"bmmd"'   .JML.    YM    `"bmmmdPY

*/

