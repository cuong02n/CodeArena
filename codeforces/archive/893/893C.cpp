/*
    author : cuong2905say
    created : 19-09-2022  13:15:55  UTC: +7
*/
#include <bits/stdc++.h>
...
class un_dir_graph {
   ...
   public:
    ...
    long long find_min_value() {
        ...
        while (!(visited.empty())) {
            min_value += __DFS(visited.begin()->first);
        }
        return min_value;
    }
...
int main() {
    ...
    int n, m;
    cin >> n >> m;
    un_dir_graph<pair<int, long long>, int> v;
    for (int i = 0; i < n; i++) {
        cin >> cost[i + 1];
        v.add_vertex_only({i + 1, cost[i + 1]});
    }

    for (int i = 0; i < m; i++) {
        long long x, y;
        cin >> x >> y;
        v.add({x, cost[x]}, {y, cost[y]});
    }
    cout << v.find_min_value() << endl;
